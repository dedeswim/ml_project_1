...
def plot_lambda_accuracy(train_errors, test_errors, lambdas, degree):
    ...
    * test_errors[0] = RMSE of the parameter found by ridge regression applied on the test set
    ...
    """
    plt.figure()
    plt.semilogx(lambdas, train_errors, color='b', marker='*', label="Train accuracy")
    plt.semilogx(lambdas, test_errors, color='r', marker='*', label="Test accuracy")
    plt.xlabel("lambda")
    plt.ylabel("accuracy")
...
def plot_poly_degree_accuracy(train_errors, test_errors, degrees, lambda_):
    ...
    * test_errors[0] = RMSE of the parameter found by ridge regression applied on the test set
    ...
    """
    plt.figure()
    plt.plot(degrees, train_errors, color='b', marker='*', label="Train accuracy")
    plt.plot(degrees, test_errors, color='r', marker='*', label="Test accuracy")
    plt.xlabel("degree")
    plt.ylabel("accuracy")
    plt.title("Ridge regression for lambda " + str(lambda_))
    leg = plt.legend(loc=1, shadow=True)
