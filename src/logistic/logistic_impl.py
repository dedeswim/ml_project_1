...
def logistic_regression(y: np.ndarray, tx: np.ndarray, w: np.ndarray, lambda_: float = 0) \
        -> Tuple[float, np.ndarray, np.ndarray]:
    """"
    Returns loss, gradient and hessian of logistic regression.
    ...
    Returns
    -------
    loss, gradient, hessian: float, np.ndarray, np.ndarray
        The loss, the gradient and the hessian for the given logistic regression parameters.
    """
...
def gradient_descent_step(y: np.ndarray, tx: np.ndarray, w: np.ndarray, gamma:np.ndarray,\
    ...
    """
    # Return loss, gradient, hessian
    loss, gradient, hessian = logistic_regression(y, tx, w, lambda_)
